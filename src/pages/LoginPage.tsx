...
import { Mail, Phone, Lock, Loader2, Leaf, ArrowLeft, Eye, EyeOff, CheckCircle2, UserCircle } from "lucide-react";
import { signUp, signInWithEmail, signInWithPhone, resendConfirmationEmail, devLogin } from "../services/auth.service";
...
function PasswordInput({ register, error, placeholder = "Password" }: { register: UseFormRegisterReturn; error?: string; placeholder?: string }) {
...
export default function LoginPage() {
	...
	async function onLoginPhone(data: LoginPhoneValues) {
		try {
			...
			toast.error(err instanceof Error ? err.message : "Nomor atau password salah");
		}
	}
	...
	async function onSignup(data: SignupValues) {
		try {
			...
			// (to prevent email enumeration). The only signal is an empty
			// `identities` array on the returned user.
			const isAlreadyRegistered =
				!!result.user && result.user.identities?.length === 0;
			...
			toast.error(err instanceof Error ? err.message : "Gagal mendaftar. Email mungkin sudah terdaftar.");
		}
	}
	...
	async function handleDevLogin(role: "member" | "admin") {
		...
		try {
			const profile = await devLogin(
				role === "admin" ? "admin@banksampah.dev" : "member@banksampah.dev",
			...
			toast.error(err instanceof Error ? err.message : `Gagal masuk sebagai ${role}. Cek kredensial di Supabase.`);
		} finally {
			setDevLoading(null);
		}
	}
		...
		<div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
			<div className="w-full max-w-sm">
				...
				<div className="flex flex-col items-center mb-8">
					...
					<h1 className="text-2xl font-black text-slate-900 tracking-tight">Bank Sampah Sembada</h1>
					<p className="text-sm text-slate-500 font-medium mt-1">
						{mode === "login" ? "Masuk ke akun Anda" : mode === "signup" ? "Buat akun baru" : "Verifikasi email Anda"}
					</p>
				</div>
				...
				<div className="bg-white rounded-3xl border border-slate-100 shadow-[0_2px_20px_-4px_rgba(0,0,0,0.06)] p-6">
						...
						<div className="flex gap-1 p-1 bg-slate-100 rounded-xl mb-6">
							<button
								...
								className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${
									loginTab === "email" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"
								}`}
							>
								Email
							</button>
							<button
								...
								className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${
									loginTab === "phone" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"
								}`}
							>
								Nomor HP
							</button>
					...
					{mode === "login" && loginTab === "email" && (
						<form onSubmit={handleLoginEmail(onLoginEmail)} className="space-y-5">
							<div>
								...
								{loginEmailErrors.email && <p className="text-xs text-red-600 font-medium mt-1">{loginEmailErrors.email.message}</p>}
							</div>

							<PasswordInput register={registerLoginEmail("password")} error={loginEmailErrors.password?.message} />
							...
							>
								{loginEmailSubmitting ? <Loader2 size={16} className="animate-spin" /> : null}
								{loginEmailSubmitting ? "Memuat..." : "Masuk"}
							</button>

							<p className="text-center text-xs text-slate-500 font-medium">
								Belum punya akun?{" "}
								<button type="button" onClick={() => setMode("signup")} className="text-brand-600 font-bold hover:text-brand-700">
									Daftar
								</button>
							</p>
						</form>
					...
					{mode === "login" && loginTab === "phone" && (
						<form onSubmit={handleLoginPhone(onLoginPhone)} className="space-y-5">
							<div>
								...
								{loginPhoneErrors.phone && <p className="text-xs text-red-600 font-medium mt-1">{loginPhoneErrors.phone.message}</p>}
							</div>

							<PasswordInput register={registerLoginPhone("password")} error={loginPhoneErrors.password?.message} />
							...
							<button
								...
								className="w-full py-2.5 bg-brand-600 text-white font-bold rounded-xl text-sm hover:bg-brand-700 disabled:opacity-60 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
							>
								{loginPhoneSubmitting ? <Loader2 size={16} className="animate-spin" /> : null}
					...
					{mode === "signup" && (
						<form onSubmit={handleSignup(onSignup)} className="space-y-5">
							<div>
								...
								{signupErrors.email && <p className="text-xs text-red-600 font-medium mt-1">{signupErrors.email.message}</p>}
							</div>

							<PasswordInput register={registerSignup("password")} error={signupErrors.password?.message} placeholder="Password (min. 6 karakter)" />
							...
							<button
								...
								className="w-full py-2.5 bg-brand-600 text-white font-bold rounded-xl text-sm hover:bg-brand-700 disabled:opacity-60 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
							>
								{signupSubmitting ? <Loader2 size={16} className="animate-spin" /> : <UserCircle size={16} />}
					...
					{mode === "verify-email" && (
						<div className="space-y-5 text-center">
							...
							<div>
								<h2 className="text-lg font-black text-slate-900">Verifikasi Email</h2>
								<p className="text-sm text-slate-500 font-medium mt-1">
									Kami telah mengirim link verifikasi ke <span className="font-bold text-slate-700">{pendingEmail}</span>
								</p>
							</div>

							<div className="p-4 bg-slate-50/50 rounded-xl border border-slate-100 text-left space-y-2">
								<p className="text-xs text-slate-500 font-medium">1. Buka email Anda</p>
								<p className="text-xs text-slate-500 font-medium">2. Klik link verifikasi dari Bank Sampah Sembada</p>
								<p className="text-xs text-slate-500 font-medium">3. Kembali ke halaman ini dan klik "Sudah Verifikasi"</p>
							</div>
							...
							<button onClick={handleResendEmail} className="text-xs text-brand-600 font-bold hover:text-brand-700">
